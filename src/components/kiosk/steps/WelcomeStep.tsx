...
export function WelcomeStep({ config, stream, onStart }: WelcomeStepProps) {
  const videoRef       = useRef<HTMLVideoElement>(null);
  const [pulse, setPulse]       = useState(false);
  const [unblurring, setUnblur] = useState(false);

  // Pulziranje CTA besedila
  useEffect(() => {
    const t = setInterval(() => setPulse((p) => !p), 1500);
    return () => clearInterval(t);
  }, []);

  // Prikaži stream v zamazanem ozadju
  useEffect(() => {
  ...
  const handleStart = () => {
    setUnblur(true);
    // Počakaj konec animacije (0.5s), nato naprej
    ...
    <div className="relative flex flex-col items-center justify-center h-full w-full select-none overflow-hidden">
      ...
      {stream && (
        <video
          ...
          style={{
            transform:  "scaleX(-1)",
            filter:     unblurring ? "blur(0px)" : "blur(24px)",
            opacity:    0.4,
      ...
      {/* Gradient overlay — vedno vidno */}
      {!stream && (
        <div
          className="absolute inset-0 opacity-20"
          style={{ background: `radial-gradient(ellipse at center, ${branding.primaryColor}88 0%, transparent 70%)` }}
        />
      )}

      {/* Dark overlay za berljivost */}
      <div
        className="absolute inset-0 transition-opacity duration-500"
        style={{ background: "rgba(0,0,0,0.55)", opacity: unblurring ? 0.1 : 1 }}
      />
      ...
      <div
        ...
        onClick={handleStart}
        style={{ opacity: unblurring ? 0 : 1, transition: "opacity 0.3s ease" }}
      >
        {/* Logo */}
        {branding.logoUrl ? (
          <img src={branding.logoUrl} alt="Logo" className="h-24 w-auto object-contain mb-10 drop-shadow-2xl" />
        ) : (
          <div
        ...
        >
          <div className="relative">
            <div className="absolute inset-0 rounded-full animate-ping opacity-30" style={{ backgroundColor: branding.primaryColor }} />
            <div
